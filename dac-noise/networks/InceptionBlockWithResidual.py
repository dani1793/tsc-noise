...
class InceptionBlockWithResidual(InceptionNetResidualBlock.InceptionNetResidualBlock):
    def __init__(self, in_channels, out_channels, nb_filter1, nb_filter2, *args, **kwargs):
        super().__init__(in_channels, out_channels, expansion =1, downsampling = 1, *args, **kwargs)
        self.blocks = nn.Sequential(inceptionModule.InceptionModule(in_channels,64,1,nb_filter1,32,32,True), 
                                    inceptionModule.InceptionModule(32,64,1,nb_filter2,32,32,True)
                                    )
